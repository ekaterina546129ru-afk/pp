...
def find_phone_by_id(phones, phone_id): 
    for i in range(len(phones)):
        if phones[i].id == phone_id:
            return i
        
    return -1
      
def delete_phone_by_id(phones, phone_id):
    index = find_phone_by_id(phones, phone_id)
    if index != -1:
        phones.pop(index)
        return True
...
def print_phone_by_id(phones, phone_id):
    index = find_phone_by_id(phones, phone_id)
    ...
    if index != -1:
        ...
        print(f"телефон с ИД {phone_id} не найден")
